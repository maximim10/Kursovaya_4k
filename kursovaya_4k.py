...
L=1; n=20; h=L/n; b=h; gamma=1/2; E1=2; E2=E1/10; points=100000; scale=1
...
def Find_stress(t):
    return k1-2*b*math.atan((t-L/2)/b)
...
if __name__ == '__main__':
    ...
    pool.close()
    pool.join()
    print("threading time  --- %s seconds ---" % (time.time() - t_th_start))
    #plt.plot(x_local, y, "y")
    plt.plot(x_local, d_e_th, "y")
    plt.plot(x_local, d1_th, "b--")
    plt.plot(x_local, d2_th, "r--")
    plt.title(f"{n} ячеек периодичности\nОтношение модулей Юнга = {E1/E2}\nСиний пунктир - первое приближение; красный пунктир - второе приближение, уточнение; желтая сплошная - аналитическое решение\nИзображено {n/scale} центральной ячейки периодичности")
    plt.subplot(3, 1, 2)
    Force_arr = list(map(Find_force, x_global))
    Stress_arr = list(map(Find_stress, x_global))
    plt.plot(x_global, Force_arr)
    plt.title("График распределенной нагрузки")
    plt.subplot(3, 1, 3)
    plt.plot(x_global, Stress_arr)
    plt.title("График напряжений")
    fig2 = plt.subplots()
    plt.subplot(1, 1, 1)
    plt.plot(x_local, d_e_th, "y")
    plt.plot(x_local, d1_th, "b--")
    plt.plot(x_local, d2_th, "r--")
    #plt.plot(x_local, w0, "g")
    plt.title(f"{n} ячеек периодичности\nОтношение модулей Юнга = {E1/E2}\nСиний пунктир - первое приближение; красный пунктир - второе приближение, уточнение; желтая сплошная - аналитическое решение\nИзображено {n/scale} центральной ячейки периодичности")
    #plt.plot(x, w0)
