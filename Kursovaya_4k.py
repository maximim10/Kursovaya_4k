...
L=1; n=3; h=L/n; b=h*1000; gamma=1/2; E1=2; E2=E1/10; points=100000; scale=1
...
if __name__ == '__main__':
    ...
    pool.join()
    pool = Pool()
    d1_th = list(map(displacement_deg_1, x_local))
    pool.close()
    pool.join()
    pool = Pool()
    d2_th = list(map(displacement_deg_2, x_local))
    pool.close()
    pool.join()
